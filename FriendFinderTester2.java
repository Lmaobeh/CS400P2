...
/**
 * @author Michael Brudos Tests the proper functionality of the FriendFinder.java, the primary class
 *         in Group Project 2. Tests are conducted using JUnit
 */
class FriendFinderTester2 {
  ...
  /**
   * Changes the curUser field to different person objects in MasterList and verifies that changes
  ...
  void testChangeCurrentUser() {
    ...
    if (app.curUser.getFriendsList() != null) {
      fail(
          "The friendlist of the curNode doesn't match the person provided (curNode friend List is populated)");;
    }
    ...
    String sherley = adam.replace("Adam", "Sherley").replace("Imaginary Lane", "Imaginary Drive")
        .replace("Dude", "Lady");
    ...
    if (!app.masterList.toString().contains(sherley)) {
      fail(
          "Sherley was not represented properly by toString method. This is likely a cause of a insert error");
    }
    if (!app.curUser.getFullName().equals(p.getFullName())) {
    ...
    if (app.curUser.getFriendsList() != null) {
      fail(
          "The friendlist of the curNode doesn't match the person provided: Sherley (curNode friend List is populated)");;
    }

  }

  /**
   * Checks the functionality of the checkMutualFriends method along with other important setters
   * and methods from the CurrentUser class
   */
  @Test
  void testMutualFriendCheck() {
    ...
    if (!app.curUser.getBio().equals("Woman")) {
      fail(
          "curUsers (Sherley) should have the bio 'Woman' after using setBio() but instead it has: "
              + app.curUser.getBio());
    }
    if (!app.curUser.getResidency().equals("Kansas")) {
      fail(
          "curUsers (Sherley) should have the residency 'Kansas' after using setResidency() but instead it has: "
              + app.curUser.getResidency());
    }
    ...
    if (!app.curUser.getBio().equals("Woman")) {
      fail(
          "curUsers (Sherley) should have the bio 'Woman' after changing users but instead it has: "
              + app.curUser.getBio());
    }
    if (!app.curUser.getResidency().equals("Kansas")) {
      fail(
          "curUsers (Sherley) should have the residency 'Kansas' after changing users but instead it has: "
              + app.curUser.getResidency());
    }
